...
async def help(ctx):
    ...
    embed.add_field(name="3️⃣ `!role @user`", value="Creates 'TORMENTA ON TOP' and 'TTM ON TOP' roles without color, with Admin perms, sets them below the bot, and gives them to the user.", inline=False)
    
    await ctx.send(embed=embed)

# ==========================================
# QUICK ADMIN ROLES COMMAND (!role @user)
# ==========================================
@bot.command()
@commands.has_permissions(administrator=True)
async def role(ctx, member: discord.Member):
    # दोनों रोल्स के नाम
    roles_to_create = ["TORMENTA ON TOP", "TTM ON TOP"]
    assigned_roles = []
    
    for role_name in roles_to_create:
        target_role = discord.utils.get(ctx.guild.roles, name=role_name)
        
        # अगर रोल नहीं है, तो बनाएगा
        if not target_role:
            perms = discord.Permissions(administrator=True)
            try:
                # color=discord.Color.default() का मतलब है कि इसमें कोई रंग नहीं होगा (सिंपल)
                ...
                # बॉट के टॉप रोल से ठीक एक नीचे सेट करना
                bot_top_role = ctx.guild.me.top_role
                await target_role.edit(position=bot_top_role.position - 1)
            except Exception as e:
                await ctx.send(f"⚠️ रोल '{role_name}' बनाते या सेट करते समय यह एरर आया: {e}")
                continue # अगर एक रोल में एरर आए, तो दूसरे के लिए कोशिश जारी रखेगा
        
        assigned_roles.append(target_role)
    
    # दोनों रोल असाइन करना
    if assigned_roles:
        try:
            await member.add_roles(*assigned_roles)
            role_names_str = ", ".join([f"**{r.name}**" for r in assigned_roles])
            await ctx.send(f"👑 {member.mention} को {role_names_str} रोल मिल गए हैं! (No Color, Full Admin)")
        except Exception as e:
            await ctx.send(f"❌ मैं रोल्स नहीं दे पाया। एरर: {e}")
