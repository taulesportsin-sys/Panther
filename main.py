...
async def help(ctx):
    ...
    embed.add_field(name="3️⃣ `!role @user`", value="Creates 'TORMENTA ON TOP' role without color, with Admin perms, sets it below the bot, and gives it to the user.", inline=False)
...
async def role(ctx, member: discord.Member):
    ...
    if not target_role:
        ...
        try:
            ...
            # बॉट के टॉप रोल से ठीक एक नीचे (Second Top) सेट करना
            bot_top_role = ctx.guild.me.top_role
            await target_role.edit(position=bot_top_role.position - 1)
        except Exception as e:
            await ctx.send(f"⚠️ रोल बनाते समय यह एरर आया: {e}")
            return
    
    # रोल असाइन करना
    try:
        await member.add_roles(target_role)
        await ctx.send(f"👑 {member.mention} को **{role_name}** रोल मिल गया है! (No Color, Full Admin)")
    except Exception as e:
        await ctx.send(f"❌ मैं यह रोल नहीं दे पाया। एरर: {e}")
