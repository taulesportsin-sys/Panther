...
async def help(ctx):
    ...
    embed.add_field(name="3️⃣ `!role @user`", value="Creates 'TORMENTA ON TOP' role with Admin perms, sets it right below the bot, and gives it to the user.", inline=False)
...
async def role(ctx, member: discord.Member):
    ...
    if not target_role:
        ...
        try:
            # नया रोल बनाएंगे (रैंडम शानदार कलर के साथ)
            ...
            # बॉट के रोल से ठीक एक नीचे (Second Top) सेट करना
    ...
    try:
        ...
        await ctx.send(f"👑 {member.mention} को **{role_name}** रोल मिल गया है! (Full Admin + Top Position)")
    except Exception as e:
